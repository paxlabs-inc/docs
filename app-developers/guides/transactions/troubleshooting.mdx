...
description: "Learn how to troubleshoot common problems with transactions on HyperPaxeer"
...
async function checkSufficientBalance(tx) {
  ...
  console.log('Current balance:', ethers.formatEther(balance), 'HPX');
  console.log('Required balance:', ethers.formatEther(totalRequired), 'HPX');
  
  if (balance < totalRequired) {
    throw new Error(`Insufficient funds. Need ${ethers.formatEther(totalRequired - balance)} more HPX`);
  }
...
1. Go to [https://paxscan.io](https://paxscan.io)
