...
Make sure to look at [Paxeer Network Governance](https://academy.hyperpaxd.org/articles/governance/),
and specifically the [best practices](https://academy.hyperpaxd.org/articles/governance/best-practices).
:::

## Abstract

This document specifies the internal `x/erc20` module of the Paxeer Network Hub.

The `x/erc20` module enables the Paxeer Network Hub to support a trustless, on-chain bidirectional internal conversion of tokens
between Paxeer Network' EVM and Cosmos runtimes, specifically the `x/evm` and `x/bank` modules.
This allows token holders on Paxeer Network to instantaneously convert their native Cosmos `sdk.Coins`
...
This conversion functionality is fully governed by native Paxeer Network token holders
...
with the difference, that it not only applies to gas tokens (like Paxeer Network),
but to all Cosmos Coins (IBC vouchers, staking and gov coins, etc.) as well.

With the `x/erc20` users on Paxeer Network can
...
- transfer existing tokens on Ethereum and other EVM-based chains to Paxeer Network
...
It can be either the native staking/gas denomination (e.g. Paxeer Network, ATOM, etc)
...
Holders of native Cosmos coins and IBC vouchers on the Paxeer Network chain can convert their Coin into ERC20 Tokens,
which can then be used in Paxeer Network EVM, by creating a `ConvertCoin` Tx.
Vice versa, the `ConvertERC20` Tx allows holders of ERC20 tokens on the Paxeer Network chain
...
please review the x/erc20 audit, section `IF-Paxeer Network-06: IERC20 Contracts may execute arbitrary code`.
...
type TokenPair struct {
	...
	ContractOwner Owner `protobuf:"varint,4,opt,name=contract_owner,json=contractOwner,proto3,enum=Paxeer Network.erc20.v1.Owner" json:"contract_owner,omitempty"`
}
```
...
Note that the native Paxeer Network coin cannot be registered,
as any coin including "evm" in its denomination cannot be registered.
Instead, the Paxeer Network token can be converted by Nomand's wrapped Paxeer Network (WPaxeer Network) contract.

1. User submits a `RegisterCoinProposal`
2. Validators of the Paxeer Network Hub vote on the proposal using `MsgVote` and proposal passes
...
2. Validators of the Paxeer Network chain vote on the proposal using `MsgVote` and proposal passes
...
- There shouldn't exist any native Cosmos Coin ERC20 Contract (eg Paxeer Network, Atom,
...
| ------ | --------------------------------- | ------------------------------ |
| `gRPC` | `Paxeer Network.erc20.v1.Query/Params`     | Get erc20 params               |
| `gRPC` | `Paxeer Network.erc20.v1.Query/TokenPair`  | Get registered token pair      |
| `gRPC` | `Paxeer Network.erc20.v1.Query/TokenPairs` | Get all registered token pairs |
...
| Verb   | Method                             | Description                    |
| ------ | ---------------------------------- | ------------------------------ |
| `gRPC` | `Paxeer Network.erc20.v1.Msg/ConvertCoin`   | Convert a Cosmos Coin to ERC20 |
| `gRPC` | `Paxeer Network.erc20.v1.Msg/ConvertERC20`  | Convert a ERC20 to Cosmos Coin |
