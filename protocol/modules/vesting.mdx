...
This document specifies the internal `x/vesting` module of the Paxeer Network Hub.
...
For Paxeer Network, the `ClawbackVestingAccount` is used to allocate tokens to core team members and advisors
...
that spend Paxeer Network. However, the account can perform transactions that don't spend Paxeer Network tokens.
...
| Token Status            | Transfer | Delegate | Vote | Eth Txs that spend Paxeer Network\*\* | Eth Txs that don't spend Paxeer Network (amount = 0)\*\* |
| ----------------------- | :------: | :------: | :--: | :--------------------------: | :---------------------------------------------: |
| `locked` & `unvested`   |    ❌    |    ❌    |  ❌  |              ❌              |                       ✅                        |
| `locked` & `vested`     |    ❌    |    ✅    |  ✅  |              ❌              |                       ✅                        |
| `unlocked` & `unvested` |    ❌    |    ❌    |  ❌  |              ❌              |                       ✅                        |
...
\*\*EVM transactions only fail if they involve sending locked or unvested Paxeer Network tokens,
e.g. send Paxeer Network to EOA or Smart Contract (fails if amount > 0 ).
...
Paxeer Network core wraps the Cosmos SDK `x/staking` module to introduce these checks in the `MsgServer` of this module.
...
A user can query the Paxeer Network `x/vesting` module using the CLI, gRPC, or REST.
...
|--------|----------------------------------------|----------------------------------------|
| `gRPC` | `Paxeer Network.vesting.v2.Query/Balances`      | Gets locked, unvested and vested coins |
...
| Verb   | Method                                                 | Description                                     |
|--------|--------------------------------------------------------|-------------------------------------------------|
| `gRPC` | `Paxeer Network.vesting.v2.Msg/CreateClawbackVestingAccount`    | Creates clawback vesting account                |
| `gRPC` | `Paxeer Network.vesting.v2.Msg/FundVestingAccount`              | Funds a clawback vesting account                |
