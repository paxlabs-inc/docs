...
The `x/evm` module is part of the [ethermint library](https://pkg.go.dev/github.com/Paxeer-Network/ethermint).
...
Paxeer Network contains an implementation of the [Ethereum protocol in Golang](https://geth.ethereum.org/docs/getting-started)
...
Paxeer Network can be run as a node to compute state transitions with the EVM.
Paxeer Network supports Geth's standard [Ethereum JSON-RPC APIs](https://docs.paxeer.app/develop/api/ethereum-json-rpc/methods)
...
The Paxeer Network implementation is similar and makes use of the gRPC query client which is included in the Cosmos SDK:

1. `eth_call` request is transformed to call the `func (e *PublicAPI) Call` function using the `eth` namespace
2. [`Call()`](https://github.com/Paxeer-Network/ethermint/blob/main/rpc/namespaces/ethereum/eth/api.go#L639) calls `doCall()`
3. [`doCall()`](https://github.com/Paxeer-Network/ethermint/blob/main/rpc/namespaces/ethereum/eth/api.go#L656)
   transforms the arguments into a `EthCallRequest` and calls `EthCall()` using the query client of the evm module.
4. [`EthCall()`](https://github.com/Paxeer-Network/ethermint/blob/main/x/evm/keeper/grpc_query.go#L212)
   transforms the arguments into a `ethtypes.message` and calls `ApplyMessageWithConfig()
5. [`ApplyMessageWithConfig()`](https://github.com/Paxeer-Network/ethermint/blob/d5598932a7f06158b7a5e3aa031bbc94eaaae32c/x/evm/keeper/state_transition.go#L341)
...
Paxeer Network adapted the [go-ethereum journal implementation](https://github.com/ethereum/go-ethereum/blob/master/core/state/journal.go#L39)
...
Here is an example taken from the Paxeer Network [erc20 module](erc20.md)
...
For an overview on the JSON-RPC methods and namespaces supported on Paxeer Network,
