...
Paxeer Network is built using [Go](https://golang.org/dl/) version `1.20+`. Check your version with:
...
Paxeer Network scripts are using [jq](https://stedolan.github.io/jq/download/) version `1.6+`. Check your version with:
...
Clone and build the Paxeer Network from source using `git`. The `<tag>` refers to a release tag on Github. Check the latest Paxeer Network
...
When it comes to using Docker with Paxeer Network, there are two options available:
Build a binary of the Paxeer Network daemon inside a dockerized build environment
or build a Docker image, that can be used to spin up individual containers running the Paxeer Network binary.
For information on how to achieve this,
proceed to the dedicated page on [working with Docker](./docker-build.md).

## Run an Paxeer Network node

To become familiar with Paxeer Network, you can run a local blockchain node that produces blocks and exposes EVM and Cosmos
...
# Example Output:
# - address: Paxeer Network19xnmslvl0pcmydu4m52h2gf0std5ee5pfgpyuf
#   name: dev0
...
To export your Paxeer Network key as an Ethereum private key (for use with [Metamask](https://academy.hyperpaxd.org/articles/beginner/connect-your-wallet/metamask)
 for example):
...
You can use hyperpaxd to query information or submit transactions on the blockchain. Queries and transactions are requests
 that you send to an Paxeer Network node through the Tendermint RPC.
...
Now that you've learned the basics of how to run and interact with an Paxeer Network network,
