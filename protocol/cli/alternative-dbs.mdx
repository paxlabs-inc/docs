...
```bash
# cd into the directory where you have the Paxeer Network protocol source code
cd Paxeer Network
...
check the tag of the `grocksdb` dependency in the `go.mod` file of the Paxeer Network repository.
...
```bash
# cd into the directory where the Paxeer Network source code is
cd Paxeer Network
...
:::note
The Paxeer Network team carried out an
