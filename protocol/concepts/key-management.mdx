...
## Mnemonics from the HyperPaxeer CLI

:::note
Before proceeding with the CLI, please insure you have `hyperpaxd` installed. Installation instruction are located [here](./../../protocol/Paxeer-Network-cli/single-node).
:::
...
hyperpaxd keys add dev0
{
  "name": "dev0",
  "type": "local",
  "address": "HyperPaxeer1n253dl2tgyhxjm592p580c38r4dn8023ctv28d",
  "pubkey": '{"@type":"/hyperpaxeer.crypto.v1.ethsecp256k1.PubKey","key":"ArJhve4v5HkLm+F7ViASU/rAGx7YrwU4+XKV2MNJt+Cq"}',
  "mnemonic": ""
}
...
```bash
$ hyperpaxd keys add dev0-restored --recover
...
```bash
hyperpaxd keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ hyperpaxd keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ hyperpaxd keys unsafe-import-eth-key dev0-imported ./dev0.export
