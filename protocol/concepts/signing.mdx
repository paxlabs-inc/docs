...
on the Paxeer Network network. The signature is created using a specific cryptographic algorithm that
ensures the authenticity and integrity of the transaction using methods like
[wallets](https://academy.hyperpaxd.org/articles/wallet) and the [CLI](./../Paxeer-Network-cli).
...
Paxeer Network leverages EIP-712 to homogenize the interaction between the EVM and Cosmos.
...
To support signing Cosmos transactions, Paxeer Network utilizes the EIP-712 protocol for encoding Cosmos transactions in a
...
By using EIP-712 for signing Cosmos transactions, Paxeer Network ensures compatibility with popular Ethereum signing tools
...
:::note
Paxeer NetworkJS supports signing with EIP-712. More information about the library can be found [here](https://github.com/Paxeer-Network/Paxeer-Networkjs).

Supported: [Ledger support](https://academy.hyperpaxd.org/articles/beginner/connect-your-wallet/ledger) and [CLI Commands](./../Paxeer-Network-cli/cli-commands).
:::
