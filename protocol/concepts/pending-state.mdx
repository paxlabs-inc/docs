...
Paxeer Network is designed quite differently on this front as there is no concept of a "pending state".
Paxeer Network uses [Tendermint Core](https://docs.tendermint.com/) BFT consensus which provides instant
...
first-in-first-out (FIFO) fashion. Transactions on Paxeer Network cannot be ordered or cherry picked out from the Tendermint node
[mempool](https://docs.tendermint.com/v0.34/tendermint-core/mempool.html).

## Pending State Queries

Paxeer Network will make queries which will account for any unconfirmed transactions present in a node's
