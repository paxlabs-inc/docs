...
Check the FAQ for running a validator on Paxeer Network.
...
Paxeer Network is powered by [Tendermint](https://docs.tendermint.com/v0.34/introduction/what-is-tendermint.html) Core,
...
Paxeer Network is a public Proof-of-Stake (PoS) blockchain, meaning that validator's weight is determined by the amount of
staking tokens (Paxeer Network) bonded as collateral. These staking tokens can be staked directly by the validator or delegated
to them by Paxeer Network holders.
...
offline, they risk their staked tokens (including Paxeer Network delegated by users) being "slashed" by the protocol to
...
Delegators are Paxeer Network holders who cannot, or do not want to run validator operations themselves. Users can delegate
Paxeer Network to a validator and obtain a part of its revenue in exchange (for more detail on how revenue is distributed, see
...
and *precommits*.
- **Validator's Address**: `Paxeer Networkvaloper1-` address. This is the address used to identify your validator publicly.
...
- **Minimum self-bond amount**: Minimum amount of Paxeer Network the validator needs to have bonded at all times. If the
validator's self-bonded stake falls below this limit, its entire staking pool will be unbonded.
- **Initial self-bond amount**: Initial amount of Paxeer Network the validator wants to self-bond.

```bash
hyperpaxd tx staking create-validator
--pubkey Paxeer Networkvalconspub1zcjduepqs5s0vddx5m65h5ntjzwd0x8g3245rgrytpds4ds7vdtlwx06mcesmnkzly
--amount "2ahpx"
--from tmp
...
Once a validator is created and registered, Paxeer Network holders can delegate Paxeer Network to it, effectively adding stake to
its pool. The total stake of a validator is the sum of the Paxeer Network self-bonded by the validator's operator and the
Paxeer Network bonded by external delegators.
...
`Paxeer Networkvalconspub`.
    - Generated when the node is created with `hyperpaxd init`.
    - Get this value with `hyperpaxd tendermint show-validator`
e.g. `Paxeer Networkvalconspub1zcjduc3qcyj09qc03elte23zwshdx92jm6ce88fgc90rtqhjx8v0608qh5ssp0w94c`
...
transactions. Application keys are associated with a public key `Paxeer Networkpub-` and an address `Paxeer Network-`. Both
are derived from account keys generated by `hyperpaxd keys add`.

:::warning
A validator's operator key is directly tied to an application key, but uses reserved prefixes solely for this
purpose: `Paxeer Networkvaloper` and `Paxeer Networkvaloperpub`
:::
...
slashed, but do not earn any rewards from their operation. It is still possible to delegate Paxeer Network to this
validator. Un-delegating from an `unbonded` validator is immediate.

Delegators have the same state as their validator.

:::warning
Delegations are not necessarily bonded. Paxeer Network can be delegated and bonded, delegated and unbonding, delegated and
unbonded, or liquid.
:::
...
The validator operator's "self-bond" refers to the amount of Paxeer Network stake delegated to itself. You can increase your
self-bond by delegating more Paxeer Network to your validator account.
...
If you want to obtain coins for the testnet, you can do so by using the [faucet](https://faucet.hyperpaxd.dev/).

</details>

<details>

<summary><b>Is there a minimum amount of Paxeer Network that must be staked to be an active (bonded) validator?</b></summary>
...
- **Amount of self-bonded Paxeer Network:** Number of Paxeer Network a validator self-bonded to its staking pool. A validator with higher
amount of self-bonded Paxeer Network has more skin in the game, making it more liable for its actions.

- **Amount of delegated Paxeer Network:** Total number of Paxeer Network delegated to a validator. A high stake shows that the community
trusts this validator, but it also means that this validator is a bigger target for hackers. Validators are expected
to become less and less attractive as their amount of delegated Paxeer Network grows. Bigger validators also increase the
...
a good practice for validators to have their setup audited by third parties. Note though, that the Paxeer Network team will
...
- **Provide oversight and feedback on correct deployment of community pool funds:** the Paxeer Network protocol includes the a
...
Staking Paxeer Network can be thought of as a safety deposit on validation activities. When a validator or a delegator wants to
...
of misbehavior). This means that every user that bonded Paxeer Network to this validator gets penalized in proportion
to its stake. Delegators are therefore incentivized to delegate to validators that they anticipate will function safely.

</details>
<details>

<summary><b>Can a validator run away with its delegators' Paxeer Network?</b></summary>

By delegating to a validator, a user delegates staking power. The more staking power a validator has, the more weight
it has in the consensus and processes. This does not mean that the validator has custody of its delegators' Paxeer Network.
...
<summary><b>How often will a validator be chosen to propose the next block? Does it go up with the quantity of Paxeer Network
staked?</b></summary>
...
bonded stake across all validators is 100 Paxeer Network, and a validator's total stake is 10 Paxeer Network, then this validator
...
- **Block rewards:** Native tokens of applications run by validators (e.g. Paxeer Network on Paxeer Network) are inflated to produce
block provisions. These provisions exist to incentivize Paxeer Network holders to bond their stake, as non-bonded Paxeer Network will
be diluted over time.
- **Transaction fees:** Paxeer Network maintains a whitelist of token that are accepted as fee payment. The initial fee token is
the `Paxeer Network`.
...
Paxeer Network enforces the parameter that each validator sets. These parameters can only be defined when initially declaring
...
This means that even though each validator gains Paxeer Network with each provision, all validators will still maintain equal
weight.

Let us take an example where we have 10 validators with equal staking power and a commission rate of 1%. Let us also
assume that the provision for a block is 1000 Paxeer Network and that each validator has 20% of self-bonded Paxeer Network. These tokens
do not go directly to the proposer. Instead, they are evenly spread among validators. So now each validator's pool
has 100 Paxeer Network. These 100 Paxeer Network will be distributed according to each participant's stake:

- Commission: `100*80%*1% = 0.8 Paxeer Network`
- Validator gets: `100\*20% + Commission = 20.8 Paxeer Network`
- All delegators get: `100\*80% - Commission = 79.2 Paxeer Network`

Then, each delegator can claim its part of the 79.2 Paxeer Network in proportion to their stake in the validator's staking pool.
Note that the validator's commission is not applied on block provisions. Note that block rewards (paid in Paxeer Network) are
...
equal stake. Each of them applies a 1% commission and has 20% of self-bonded Paxeer Network. Now comes a successful block
that collects a total of 1005 Paxeer Network in fees. Let's assume that the proposer included 100% of the signatures in its
...
- For the proposer validator:
    ...
    - The pool obtains $R ~ + ~ 5\%(R)$: 105 Paxeer Network
    - Commission: $105 ~ *~ 80\% ~* ~ 1\%$ = 0.84 Paxeer Network
    - Validator's reward: $105 ~ * ~ 20\% ~ + ~ Commission$ = 21.84 Paxeer Network
    - Delegators' rewards: $105 ~ * ~ 80\% ~ - ~ Commission$ = 83.16 Paxeer Network \(each delegator will be able to claim its portion
of these rewards in proportion to their stake\)

    - The pool obtains $R$: 100 Paxeer Network
    - Commission: $100 ~ *~ 80\% ~* ~ 1\%$ = 0.8 Paxeer Network
    - Validator's reward: $100 ~ * ~ 20\% ~ + ~ Commission$ = 20.8 Paxeer Network
    - Delegators' rewards: $100 ~ * ~ 80\% ~ - ~ Commission$ = 79.2 Paxeer Network \(each delegator will be able to claim its portion
of these rewards in proportion to their stake\
...
- [Learnings from BlockDaemon](https://blockdaemon.com/documentation/Paxeer-Network-post-mortem/)
...
There is an awesome guide. Polkachu is a validator on Paxeer Network and they have wrote [this page](https://github.com/polkachu/validator-guide/blob/main/validator_server_migration_best_practice.md)
to help out.
...
<summary><b>Do validators need to self-bond Paxeer Network?</b></summary>
...
Even though there is no obligation for validators to self-bond Paxeer Network, delegators should want their validator to have
self-bonded Paxeer Network in their staking pool. In other words, validators should have skin-in-the-game.

In order for delegators to have some guarantee about how much skin-in-the-game their validator has, the latter can signal
a minimum amount of self-bonded Paxeer Network. If a validator's self-bond goes below the limit that it predefined, this
...
much mining power the community usually stops contributing to that pool. Paxeer Network will rely on the same effect initially.
...
In addition to running an Paxeer Network node, validators should develop monitoring, alerting and management solutions.
...
Paxeer Network has the capacity for very high throughput compared to chains like Ethereum or Bitcoin.
...
The Paxeer Network team does not recommend one solution above the other. The community is encouraged to bolster the effort to
