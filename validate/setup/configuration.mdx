...
```toml
# In ~/.hyperpaxd/config/config.toml
...
In `~/.hyperpaxd/config/config.toml` you can set your peers.
...
by adjusting the corresponding setting in the [node configuration](https://github.com/Paxeer-Network/Paxeer-Network/blob/v18.1.0/server/config/toml.go#L74-L76):

```toml
# in $HOME/.hyperpaxd/config/config.toml
