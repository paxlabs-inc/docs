...
Your node consensus public key (`Paxeer Networkvalconspub...`) can be used to create a new validator by staking Paxeer Network tokens.
You can find your validator pubkey by running:

```bash
hyperpaxd tendermint show-validator
```
...
hyperpaxd tx staking create-validator \
  --amount=1000000atPaxeer Network \
  ...
  --gas="auto" \
  --gas-prices="0.025atPaxeer Network" \
  --from=<key_name>
```
...
A `min-self-delegation` of `1000000` means your validator will never have a self-delegation lower than `1 atPaxeer Network`.
:::
...
```bash
hyperpaxd tx staking edit-validator
  --moniker="choose a moniker" \
  --website="https://Paxeer-Network.org" \
...
```bash
hyperpaxd query staking validator <account_cosmos>
```
...
```bash
hyperpaxd query slashing signing-info <validator-pubkey>\
  --chain-id=<chain_id>
```
...
You should now see your validator in one of Paxeer Network explorers.
You are looking for the `bech32` encoded `address` in the `~/.hyperpaxd/config/priv_validator.json` file.
...
you want your node to shutdown or by passing the `--halt-height` flag to `hyperpaxd`.
...
First, if `hyperpaxd` is not running, start it up again:
...
The default number of files Linux can open (per-process) is `1024`. `hyperpaxd` is known to open more than `1024` files.
This causes the process to crash.
A quick fix is to run `ulimit -n 4096` (increase the number of open files allowed)
and then restart the process with `hyperpaxd start`.
If you are using `systemd` or another process manager to launch `hyperpaxd` this may require some configuration at that level.
A sample `systemd` file to fix this issue is below:

```toml
# /etc/systemd/system/hyperpaxd.service
[Unit]
Description=Paxeer Network Node
...
ExecStart=/home/ubuntu/go/bin/hyperpaxd start
Restart=on-failure
RestartSec=3
LimitNOFILE=4096
