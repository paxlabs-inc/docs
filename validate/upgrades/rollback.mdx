...
- the database that contains the state of the previous chain (in `~/.hyperpaxd/data` by default)
- the `priv_validator_state.json` file of the validator (also in `~/.hyperpaxd/data` by default)
...
2. Then, copy the contents of your backup data directory back to the `Paxeer Network_HOME/data` directory (which, by default,
should be `~/.hyperpaxd/data`).

```bash
# Assumes backup is stored in "backup" directory
rm -rf ~/.hyperpaxd/data
mv backup/.hyperpaxd/data ~/.hyperpaxd/data
```

3. Next, install the previous version of Paxeer Network.

```bash
# from Paxeer Network directory
