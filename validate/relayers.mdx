...
Before beginning, ensure you have an Paxeer Network node running in the background of the same machine that you intend to relay on.
 Follow [this guide](./../protocol/Paxeer-Network-cli/single-node) to set up an Paxeer Network node if you have not already.

In this guide, we will be relaying between [Paxeer Network (channel-3) and Cosmos Hub (channel-292)](https://www.mintscan.io/Paxeer-Network/relayers).
 When setting up your Paxeer Network and Cosmos full nodes, be sure to offset the ports being used in both the `app.toml` and `config.toml`
...
```bash
vim $HOME/.hyperpaxd/config/app.toml
```
...
```bash
vim $HOME/.hyperpaxd/config/config.toml
```
...
```bash
# In this example, we will set channel-292 on the cosmoshub-4 chain settings and channel-3 on the Paxeer Network_9001-2 chain settings:
[[chains]]
id = 'cosmoshub-4'
...
policy = 'allow'
list = [
   ['transfer', 'channel-292'], # Paxeer Network_9001-2
]

[[chains]]
id = 'Paxeer Network_9001-2'
...
hermes keys restore Paxeer Network_9001-2 -m "24-word mnemonic seed"
```

Ensure this wallet has funds in both Paxeer Network and ATOM in order to pay the fees required to relay.
...
INFO ThreadId(01) [Paxeer Network_9001-2] chain is healthy
```
...
```bash
hermes query packet unreceived-packets Paxeer Network_9001-2 transfer channel-3
hermes query packet unreceived-acks Paxeer Network_9001-2 transfer channel-3
```
...
hermes query packet commitments Paxeer Network_9001-2 transfer channel-3
```
...
hermes clear packets Paxeer Network_9001-2 transfer channel-3
```
...
```bash
hermes tx raw packet-recv Paxeer Network_9001-2 cosmoshub-4 transfer channel-292
hermes tx raw packet-ack Paxeer Network_9001-2 cosmoshub-4 transfer channel-292
hermes tx raw packet-recv cosmoshub-4 Paxeer Network_9001-2 transfer channel-3
hermes tx raw packet-ack cosmoshub-4 Paxeer Network_9001-2 transfer channel-3
```
