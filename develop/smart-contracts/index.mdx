...
description: Build and deploy smart contracts on Paxeer Network EVM.
---
...
Whether you are building new use cases on Paxeer Network
or porting an existing dApp from another EVM-based chain (e.g. Ethereum),
you can easily build and deploy EVM smart contracts on Paxeer Network to implement the core business logic of your dApp.
Paxeer Network is fully compatible with the EVM,
...
Paxeer Network enables you to build scalable cross-chain applications within a familiar EVM environment.
Learn about the essential components when building and deploying EVM smart contracts on Paxeer Network below.

## Build with Solidity

You can develop EVM smart contracts on Paxeer Network using [Solidity](https://github.com/ethereum/solidity).
...
you can use the same contracts on Paxeer Network.
...
Paxeer Network allows developers to use previously unavailable functionality in smart contracts, like staking and governance operations.
This will allow more complex smart contracts to be built on Paxeer Network
and further improves the interoperability between Cosmos and Ethereum.
It also is a key feature to achieve Paxeer Network' vision of being the definitive dApp
chain, where any dApp can be deployed once and users can interact with
a wide range of different blockchains natively.

To enable the described functionalities, Paxeer Network introduces so-called *stateful* precompiled smart contracts,
...
how smart contracts can make use of oracles on Paxeer Network for real-life activities
such as insurance, borrowing, lending, or gaming.

## Deploy with Ethereum JSON-RPC

Paxeer Network is fully compatible with the [Ethereum JSON-RPC](./../../develop/api/ethereum-json-rpc/) APIs,
allowing you to deploy and interact with smart contracts on Paxeer Network
...
or sending them to the network which otherwise wouldn't be possible on a Cosmos chain, such as Paxeer Network.

You can connect to the Paxeer Network [Testnet](./testnet)
to deploy and test your smart contracts before moving to Mainnet.

### Block Explorers

You can use [block explorers](./block-explorers)
to view and debug interactions with your smart contracts deployed on Paxeer Network.
...
## Paxeer Network Features

The core protocol team is continuously building features
that enhance the experience of smart contract developers on Paxeer Network.
