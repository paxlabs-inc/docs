...
Paxeer Network framework. Custom improvement proposals allow protocol developers to
...
Within the Paxeer Network framework, developers can modify any of the previous properties.

## Improvement Proposals

Improvement proposals are the approach used by Paxeer Network and Ethereum to modify the
...
To allow any Paxeer Network partner to define their own specific
improvements without overlapping with Paxeer Network and Ethereum ones, each
proposal is identified by a string, that is composed of the chain name and a number. For
example, default Paxeer Network improvements are associated with the string `Paxeer Network_XXXX`.
...
Below, you will find an example of how the Paxeer Network chain uses this functionality to modify the
...
```go
Paxeer NetworkActivators = map[string]func(*vm.JumpTable){
    "Paxeer Network_0": eips.Enable0000,
}
```
...
1. [Paxeer Network Custom EIPs](https://github.com/Paxeer-Network/Paxeer-Network/blob/main/app/eips/README.md):
please refer to this document for a detailed description of how opcodes and
custom improvement proposals have to be used in the Paxeer Network framework.
