...
description: "Learn how transactions are processed on HyperPaxeer"
icon: "diagram-project"
---

## Overview

This guide explains the transaction flow process on HyperPaxeer from submission to finality.
...
<Steps>
  ...
  <Step title="Finalization">
    After block inclusion, transaction has deterministic CometBFT finality
...
**Official report:** 277 ms average block time, 341 ms p95
...
```solidity
// Before transaction
state[address].balance = 100 HPX

// Transaction: transfer 10 HPX
transfer(recipient, 10 HPX)

// After transaction
state[address].balance = 90 HPX
state[recipient].balance = originalBalance + 10 HPX
```
...
| Finality Time | Instant deterministic finality |
